...
export default function ClientLayout({ children }) {
  ...
  const { t, locale, toggleLanguage, cartCount, restaurant } = useApp();
    ...
    <div className="min-h-screen bg-black flex flex-col">
      <header className="sticky top-0 z-50 bg-black/90 backdrop-blur border-b border-white/10 px-4 py-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-bold" style={{ color: brandColor }}>{brandName}</h1>
        </div>
        <div className="flex items-center gap-3">
          <button onClick={toggleLanguage} className="p-2 text-white/40 hover:text-wave-500 transition-colors text-xs font-medium">
            <Globe size={16} className="inline mr-0.5" />
            {locale === 'fr' ? 'EN' : 'FR'}
          </button>
          <button onClick={() => navigate('/server-call')} className="p-2 text-white/70 hover:text-wave-500 transition-colors">
            <Phone size={20} />
          </button>
          <button onClick={() => navigate('/cart')} className="relative p-2 text-white/70 hover:text-gold-500 transition-colors">
