...
export default function CartPage() {
  ...
  const { cart, cartTotal, updateQuantity, removeFromCart, t, restaurantSlug, tableId } = useApp();
  ...
  return (
    <div className="min-h-screen bg-black pb-32">
      ...
      <div className="fixed bottom-16 left-0 right-0 p-4 bg-black/90 backdrop-blur border-t border-white/10 space-y-2">
        <Button onClick={() => navigate('/checkout')} className="w-full">
          {t('Proceed to Checkout')}
        </Button>
        <Button variant="ghost" onClick={() => { if (base) navigate(`${base}/menu`); else navigate(-1); }} className="w-full text-sm">
          {t('Continue Shopping')}
        </Button>
      </div>
